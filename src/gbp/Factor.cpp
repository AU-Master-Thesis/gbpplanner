...
//  - A vector of pointers to Variables that the factor is to be connected to.
//  Note, the order of the variables matters.
//  - sigma: factor strength. The factor precision Lambda = sigma^-2 * Identity
//  - measurement z: Eigen::VectorXd, must be same size as the output of the
//  measurement function h().
//  - n_dofs is the number of degrees of freedom of the variables this factor is
//  connected to. (eg. 4 for [x,y,xdot,ydot])
/*****************************************************************************************************/
Factor::Factor(int f_id, int r_id,
               std::vector<std::shared_ptr<Variable>> variables, float sigma,
               const Eigen::VectorXd &measurement, int n_dofs)
    : f_id_(f_id), r_id_(r_id), key_(r_id, f_id), variables_(variables),
      z_(measurement), n_dofs_(n_dofs) {

  // Initialise precision of the measurement function
  this->meas_model_lambda_ =
      Eigen::MatrixXd::Identity(z_.rows(), z_.rows()) / pow(sigma, 2.);
...
/*****************************************************************************************************/
// Drawing function for the factor. Draws a 3d Cylinder (line-ish) between its
// connected variables
/*****************************************************************************************************/
void Factor::draw() {
  if ((factor_type_ == DYNAMICS_FACTOR && globals.DRAW_PATH)) {
    ...
    DrawCylinderEx(
        Vector3{(float)v_0->mu_(0), globals.ROBOT_RADIUS, (float)v_0->mu_(1)},
...
/*****************************************************************************************************/
// Default measurement function h_func_() is the identity function: it returns
// the variable.
/*****************************************************************************************************/
Eigen::MatrixXd h_func_(const Eigen::VectorXd &X) { return X; };
/*****************************************************************************************************/
// Default measurement function Jacobian J_func_() is the first order taylor
// series jacobian by default. When defining new factors, custom h_func_() and
// J_func_() must be defined, otherwise defaults are used.
/*****************************************************************************************************/
Eigen::MatrixXd Factor::J_func_(const Eigen::VectorXd &X) {
  return this->jacobianFirstOrder(X);
};
...
Eigen::MatrixXd Factor::jacobianFirstOrder(const Eigen::VectorXd &X0) {
  ...
  for (int i = 0; i < X0.size(); i++) {
    ...
    jac_out(Eigen::all, i) =
        (h_func_(X_copy) - h0) / delta_jac; // Derivative (first order)
  }
  return jac_out;
};
...
// Messages from connected variables are aggregated. The beliefs are used to
// create the linearisation point X_. The Factor potential is calculated using
// h_func_ and J_func_ The factor precision and information is created, and then
// marginalised to create outgoing messages to its connected variables.
/*****************************************************************************************************/
bool Factor::update_factor() {
  ...
  // eg. to avoid extra computation, factor may not be required if two connected
  // variables are too far apart. in which case send out a Zero Message.
  if (this->skip_factor()) {
    for (auto var : variables_) {
  ...
  // The Factor potential and linearised Factor Precision and Information is
  // calculated using h_func_ and J_func_ residual() is by default (z -
  // h_func_(X)) Skip calculation of Jacobian if the factor is linear and
  // Jacobian has already been computed once
  h_ = h_func_(X_);
  J_ = (this->linear_ && this->initialised_) ? J_ : this->J_func_(X_);
  Eigen::MatrixXd factor_lam_potential =
      J_.transpose() * meas_model_lambda_ * J_;
  Eigen::VectorXd factor_eta_potential =
      (J_.transpose() * meas_model_lambda_) * (J_ * X_ + residual());
  ...
  for (int v_out_idx = 0; v_out_idx < variables_.size(); v_out_idx++) {
    ...
    // Combine the factor with the belief from other variables apart from the
    // receiving variable
    int idx_v = 0;
    ...
    // Marginalise the Factor Precision and Information to send to the relevant
    // variable
    outbox_[var_out->key_] = marginalise_factor_dist(
        factor_eta, factor_lam, v_out_idx, marginalisation_idx);
...
/*****************************************************************************************************/
// Marginalise the factor Precision and Information and create the outgoing
// message to the variable
/*****************************************************************************************************/
Message Factor::marginalise_factor_dist(const Eigen::VectorXd &eta,
                                        const Eigen::MatrixXd &Lam, int var_idx,
                                        int marg_idx) {
  ...
  Eigen::MatrixXd lam_ba(Lam.rows() - n_dofs, n_dofs),
      lam_bb(Lam.rows() - n_dofs, Lam.cols() - n_dofs);
  ...
  lam_bb << Lam(seq(0, marg_idx - 1), seq(0, marg_idx - 1)),
      Lam(seq(0, marg_idx - 1), seq(marg_idx + n_dofs, last)),
      Lam(seq(marg_idx + n_dofs, last), seq(0, marg_idx - 1)),
      Lam(seq(marg_idx + n_dofs, last), seq(marg_idx + n_dofs, last));
...
// You may create a new factor_type_, in the enum in Factor.h (optional, default
// type is DEFAULT_FACTOR) Create a measurement function h_func_() and
// optionally Jacobian J_func_().

/********************************************************************************************/
/* Dynamics factor: constant-velocity model */
/*****************************************************************************************************/
DynamicsFactor::DynamicsFactor(int f_id, int r_id,
  ...
  J_ << I, dt * I, -1 * I, O, O, I, O, -1 * I;
};

Eigen::MatrixXd DynamicsFactor::h_func_(const Eigen::VectorXd &X) {
  return J_ * X;
}
Eigen::MatrixXd DynamicsFactor::J_func_(const Eigen::VectorXd &X) { return J_; }

/********************************************************************************************/
/* Interrobot factor: for avoidance of other robots */
// This factor results in a high energy or cost if two robots are planning to be
// in the same position at the same timestep (collision). This factor is created
// between variables of two robots. The factor has 0 energy if the variables are
// further away than the safety distance. skip_ = true in this case.
/********************************************************************************************/

InterrobotFactor::InterrobotFactor(
    int f_id, int r_id, std::vector<std::shared_ptr<Variable>> variables,
    float sigma, const Eigen::VectorXd &measurement, float robot_radius)
    : Factor{f_id, r_id, variables, sigma, measurement} {
...
Eigen::MatrixXd InterrobotFactor::h_func_(const Eigen::VectorXd &X) {
  ...
  Eigen::VectorXd X_diff =
      X(seqN(0, n_dofs_ / 2)) - X(seqN(n_dofs_, n_dofs_ / 2));
...
Eigen::MatrixXd InterrobotFactor::J_func_(const Eigen::VectorXd &X) {
  ...
  X_diff += 1e-6 * r_id_ *
            Eigen::VectorXd::Ones(
                n_dofs_ / 2); // Add a tiny random offset to avoid div/0 errors
  double r = X_diff.norm();
  if (r <= safety_distance_) {
...
bool InterrobotFactor::skip_factor() {
  this->skip_flag = ((X_(seqN(0, n_dofs_ / 2)) - X_(seqN(n_dofs_, n_dofs_ / 2)))
                         .squaredNorm() >= safety_distance_ * safety_distance_);
  return this->skip_flag;
}

/********************************************************************************************/
// Obstacle factor for static obstacles in the scene. This factor takes a
// pointer to the obstacle image from the Simulator. Note. in the obstacle
// image, white areas represent obstacles (as they have a value of 1). The input
// image to the simulator is opposite, which is why it needs to be inverted. The
// delta used in the first order jacobian calculation is chosen such that it
// represents one pixel in the image.
/********************************************************************************************/
ObstacleFactor::ObstacleFactor(Simulator *sim, int f_id, int r_id,
                               std::vector<std::shared_ptr<Variable>> variables,
                               float sigma, const Eigen::VectorXd &measurement,
                               Image *p_obstacleImage)
    : Factor{f_id, r_id, variables, sigma, measurement},
      p_obstacleImage_(p_obstacleImage) {
  factor_type_ = OBSTACLE_FACTOR;
  this->delta_jac =
      1. * (float)globals.WORLD_SZ / (float)p_obstacleImage->width;
};
Eigen::MatrixXd ObstacleFactor::h_func_(const Eigen::VectorXd &X) {
  ...
  auto pixel_x = (int)((X(0) + globals.WORLD_SZ / 2) * scale);
  auto pixel_y = (int)((X(1) + globals.WORLD_SZ / 2) * scale);

  Vector3 c_hsv = ColorToHSV(GetImageColor(
      *p_obstacleImage_, (int)((X(0) + globals.WORLD_SZ / 2) * scale),
      (int)((X(1) + globals.WORLD_SZ / 2) * scale)));
  h(0) = c_hsv.z;

  // auto a = 1.0;
  // this->last_measurement_.pos = Vector2{X(0) * a, X(1) * a};
  this->last_measurement_.pos = Vector2{X(0), X(1)};
  // this->last_measurement_.pos = Vector2{pixel_x, pixel_y};
  this->last_measurement_.value = c_hsv.z;

  // std::cout << "c_hsv.z = " << c_hsv.z << '\n';
