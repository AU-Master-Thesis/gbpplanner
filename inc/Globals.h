...
/***********************************************************************************************/
// Global structure. The values here are mostly set using the provided
// config.json file.
/***********************************************************************************************/
class Globals {
public:
  // Basic parameters
  const char *WINDOW_TITLE = "Distributing Multirobot Motion Planning with "
  ...
  MODES_LIST LAST_SIM_MODE =
      Timestep; // Storage of Simulation mode (if time is paused eg.)
  ...
  float COMMUNICATION_RADIUS; // [m] Inter-robot factors created if robots are
                              // within this range of each other
  float MAX_SPEED;            // [m/s]
  float
      COMMS_FAILURE_RATE; // Proportion of robots [0,1] that do not communicate
  // NOTE: changed here for experiment config lm-{1,3}, tk=13.33
  int LOOKAHEAD_MULTIPLE =
      3; // Parameter affecting how planned path is spaced out in time
  ...
  float SIGMA_POSE_FIXED =
      1e-15; // Sigma for Unary pose factor on current and horizon states
